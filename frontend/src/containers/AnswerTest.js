...
export default function AnswerTest({test}) {
  ...
  const submitTest = (e) => {
    e.preventDefault()
    axios.post('https://owe6jjn5we.execute-api.us-east-1.amazonaws.com/dev/pass-test',{answers,recruiterId: test.recruiter_id, testId: test.test_id, ["testName"]: test.test_name, username: localStorage.getItem('currentUsername')}).then(() => {
